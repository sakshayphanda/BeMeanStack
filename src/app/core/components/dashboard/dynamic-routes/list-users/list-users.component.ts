...
import {
  FriendRequestApi,
  ListAllUsersApi,
} from 'src/app/store/actions/users/users.actions';
...
export class ListUsersComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.store.select(getAllUsers).subscribe((users) => {
      if (users) {
        ...
        this.allUsersExceptCurrent = this.allUsersExceptCurrent.map((user) => {
          const newObj = Object.assign({}, user);
          const alreadyAdded = this.currentUser.friends?.some(
            (u) => u._id === user._id
          );
          if (alreadyAdded) {
    ...
    this.store.select(friendRequestSuccess).subscribe((user) => {
  ...
  checkIfSent(user) {
    let pending = false;
    this.currentUser.friendRequestsPending?.forEach((item) => {
  ...
  checkIfReceived(user) {
    ...
    this.currentUser.friendRequests?.forEach((item) => {
      if (item._id === user._id) {
        received = true;
      }
    });
